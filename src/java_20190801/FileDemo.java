...
public class FileDemo {
	...
	public void changeFile(String path) {
		// path => c:\dev\test\jdk.exe => c:\dev\test\1231141.exe
		File f1 = new File(path);
		// f1.getPath() => 전체경로 + 파일이름
		String fileName = f1.getName(); // 전체 경로중에 파일이름만 반환한다.
	...
	public static void main(String[] args) {
		...
		f2.renameTo(f3); // f2파일을 f3파일 이름으로 변경한다.
//		f1.changeFile("C:\\dev\\test\\jdk.exe");
		...
		for (String list : lists) {
			...
			if (f6.isDirectory()) {
				...
				System.out.println(name + "  " + fileSize + "byte" + "  " + lastModified);
			}
